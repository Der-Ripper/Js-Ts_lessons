...
app.use("/todos", async (req: any, res: any, next: any) => {
    try {
        ...
        const token = await Token.findByPk(req.headers['authentication']);
        console.log(token);
...
app.get("/todos", async (req: any, res: any) => {
    try {
        ...
        const todoList = await ToDo.findAll({where: {userId: req.headers['userid']}});
...
app.post("/auth", async (req: any, res: any) => {
    try{
        ...
        if(userFind) {
            ...
            await res.status(200).json({token: token.value, userId: token.userId, firstName: userFind.firstName, lastName: userFind.lastName});
        } else {
